...
* **Tính năng Tự động Đồng bộ (Auto-sync)**: Hệ thống đã tích hợp cơ chế tự động đẩy điểm của sinh viên lên Supabase ngay sau khi Worker chấm xong (nếu biến cấu hình `Supabase:AutoSync` được bật là `true`). Do đó, endpoint này chỉ dùng khi giảng viên muốn đồng bộ lại hàng loạt hoặc đồng bộ thủ công khi có sự điều chỉnh điểm số.
